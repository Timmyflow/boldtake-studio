...
describe("scroll-scrub website landing contract", () => {
  ...
  test("keeps the old workspace URL as a portfolio redirect", () => {
    ...
    expect(landingRoute).toContain("StudioOpening");
    expect(appRoute).toContain('createFileRoute("/app")');
    expect(appRoute).toContain('redirect({ to: "/"');
    expect(appRoute).not.toContain("CustomTemplate");
  });
